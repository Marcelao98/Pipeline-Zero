...
- Capítulo 3 de `1-que-porra-e-essa/` (roadmap) atualizado: item de SQL virou "Fontes de dado (começando por SQL)", deixando espaço pra outras fontes no futuro (Excel, API, arquivo); item de Python passou a usar a analogia da caixa de ferramentas; avisos de módulo de SQL e Python agora linkam pros módulos reais; entrou um aviso de que o próprio roadmap está em construção. **[decidido, escrito]**
